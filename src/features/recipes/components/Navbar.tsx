...
const Navbar = () => {
  ...
  // Sync nav search input with current URL ?search= param
  useEffect(() => {
  ...
  return (
    <nav className="site-nav">
      ...
      <div className="nav-links">
        ...
        <Link to="/recipes" className={location.pathname === '/recipes' || location.pathname.startsWith('/recipe') ? 'active' : ''}>RECIPES</Link>
        <Link to="/tips" className={location.pathname === '/tips' ? 'active' : ''}>COOKING TIPS</Link>
        <Link to="/about" className={location.pathname === '/about' ? 'active' : ''}>ABOUT US</Link>
      </div>

      <form
        className={`nav-search-pill${searchOpen ? ' is-open' : ' is-collapsed'}`}
        onSubmit={submitSearch}
        role="search"
      >
        <button
          type="button"
          className="nav-search-pill-btn"
        ...
        {searchOpen && (
          <input
            type="search"
            className="nav-search-pill-input"
            value={query}
        ...
        <div className="mobile-menu">
          <div className="mobile-menu-panel">
            <div className="mobile-menu-header">
              <Link to="/" className="brand brand-dark" aria-label="Cooks Delight home" onClick={() => setIsOpen(false)}>
                <div className="brand-mark"><span></span></div>
                <span>Cooks<br />Delight</span>
              </Link>
              <button onClick={() => setIsOpen(false)} className="icon-button menu-close" aria-label="Close menu">
                <CloseIcon />
              </button>
            </div>
            ...
            <form className="mobile-search-form" onSubmit={(e) => { submitSearch(e); setIsOpen(false); }}>
              <SearchIcon />
              <input
                type="search"
                value={query}
            ...
            <div className="mobile-menu-actions">
              <Link to="/login" className="menu-signup" onClick={() => setIsOpen(false)}>SIGN IN NOW!</Link>
            </div>
